...
/**********************************
 ** Matt's cheesey shadow buffer **
 **********************************/


# if !MAX3
class CheeseyShadowBuffer : public ShadowBuffer
{
private:
	Bitmap	*bm;
	float *zbuffer;
	...
	INode *node;
public:
	CheeseyShadowBuffer() { bm = NULL; }
	~CheeseyShadowBuffer() { if (bm) bm->DeleteThis(); }
	void DeleteThis() { delete this; }

	virtual int Update (const ObjLightDesc& light, 
						const RendContext& rendCntxt,
						Matrix3& lightTM,
						int size, float aspect, float param,
						float bias, float sampSize, ULONG flags, float clipDist=DONT_CLIP)
	{
		this->size = size;
		ls = light.ls;
		this->lightTM = lightTM;
		node = light.inode;
		/*
		...
		bm = TheManager->Create (&info);
		bm->CreateChannels (BMM_CHAN_Z);

		ULONG type;
		zbuffer = (float *)bm->GetChannel (BMM_CHAN_Z,type);
	...
	int UpdateViewDepParams (const Matrix3 &worldToCam)
	{
		...
		/*
		 * Now create a renderer instance to render from
		 */
		Renderer *rend = (Renderer *)iface->CreateInstance (SClass_ID(RENDERER_CLASS_ID), Class_ID(SREND_CLASS_ID,0));
		assert (rend);

		ViewParams view;
		view.prevAffineTM = view.affineTM = lightTM;
		view.projType = PROJ_PERSPECTIVE;
		view.hither = 1.f;
		view.yon = ls.useAtten? ls.attenEnd : 10000.f;
		view.zoom = (float)size / 400.f;
		RendParams rParams;
		rParams.rendType = RENDTYPE_NORMAL;
		rend->Open (iface->GetRootNode(), node, &view, rParams, NULL);

		/*
		 * Render the scene
		 */
#if 0		
		FrameRendParams parms;
		rend->Render (0, bm, parms, NULL);
#endif
		rend->Close(NULL);
	...
	{
		float dist = zbuffer[y*size+x];
		return dist > z? FALSE : TRUE;
	}
	float Sample (float x, float y, float z, float xslope, float yslope)
	{
		return QuickSample (int(x), int(y), z) ? 1.f : 0.f;
	}
	float FiltSample (int x, int y, float z, int level)
	{
		return QuickSample (x, y, z) ? 1.f : 0.f;
	}
	float LineSample (int x1, int y1, float z1, int x2, int y2, float z2)
	{
		return 0.f;
	}
};
...
/*****************************
 ** Matt's cheesey quadtree **
 *****************************/

#define SHAD_TOLER (0.01f)

BOOL sceneShadowEnum (INode *i, Point3 lightPos, Point3 pt)
{
	Object *o;
	o = i->EvalWorldState(0).obj;
	if (o && o->IsRenderable() && !o->IsConstObject()) {
		float d;
		Point3 lInObj, pInObj, Norm;
		TCHAR *name = o->GetObjectName();
		Ray ray;
		Matrix3 rayToObject = Inverse (i->GetObjectTM(0));
		ray.p = lInObj = rayToObject * lightPos;
		pInObj = rayToObject * pt;
		ray.dir = Normalize (pInObj - lInObj);
		if (o->IntersectRay (0, ray, d, Norm)) {
			float dist = Length (pInObj - lInObj) - SHAD_TOLER;
			if (d != 0.f && d < dist)
				return TRUE;
		}
	}

	for (int a=0; a < i->NumberOfChildren(); ++a) {
		if (sceneShadowEnum(i->GetChildNode(a), lightPos, pt))
			return TRUE;
	}
	return FALSE;
}
...
#if !MAX3
class CheeseyShadowQuadTree : public ShadowQuadTree
{
private:
	Point3		lightPos;
	int			called;
public:
	CheeseyShadowQuadTree() {}
	virtual int Update (const ObjLightDesc &light,
						const RendContext& rendCntxt,
	...
	{
		lightPos = light.lightToWorld * light.lightPos;
		called = 0;
		return 1;
	}
	virtual int UpdateViewDepParams(const Matrix3& worldToCam)
	{
		lightPos = worldToCam * lightPos;
		return 1;
	}
	BOOL inShadow (Point3 p, Point3 lightPos)
	{
		INode* scene = GetCOREInterface()->GetRootNode();
		for (int i=0; i<scene->NumberOfChildren(); i++) {
			if (sceneShadowEnum(scene->GetChildNode(i), lightPos, p))
				return TRUE;
		}
		return FALSE;
	}
	virtual float Sample(ShadeContext& sc, Point3 p, Color& color)
	{
		float retVal = 1.f;
		if (inShadow (p, lightPos)) {
			/*
			retVal -= (1.f/9.f);
			float amount = Length (p - lightPos) * 0.5f;
			for (int x = -1; x < 2; x+=2) {
				for (int y = -1; y < 2; y+=2) {
					for (int z = -1; z < 2; z+=2) {
						Point3 newP;
						newP.x = lightPos.x + float(x) * amount;
						newP.y = lightPos.y + float(y) * amount;
						newP.z = lightPos.z + float(z) * amount;
						if (inShadow (p, newP))
							retVal -= (1.f/9.f);
					}
				}
...
class DefObjLight : public ObjLightDesc 
{
	public:
		...
		void DeleteThis() {delete this;}
		int Update(TimeValue t, const RendContext& rc, BOOL shadows);
		int UpdateViewDepParams(const Matrix3& worldToCam);
...
BOOL calcMixedVertexColors(INode* node, TimeValue t, int lightModel, ColorTab& vxColTab, EvalColProgressCallback* fn)
{
	...
	// Setup ambient light
	sc.SetAmbientLight(GetCOREInterface()->GetAmbient (0, FOREVER));
	...
	*/
	...
	for (unsigned int v = 0; v < (unsigned)numVerts; v++) {
		...
		// to be realloc'ed, allocate extra space for more points.
		vxColTab.Append(1, &vxCol, vxColTab.Count());
	}
...
int DefObjLight::Update(TimeValue t, const RendContext& rc, BOOL shadows)
{
...
void SContext::UpdateLights()
{
	for (int i=0; i<lightTab.Count(); i++) {
		#if MAX3
			((LightInfo*)lightTab[i])->lightDesc->Update(t, rc, NULL, shadow, FALSE);
		#else
			((LightInfo*)lightTab[i])->lightDesc->Update(t, rc, shadow);
		#endif
...
{
	return new CheeseyShadowBuffer();
}
...
void sceneLightEnum(INode* node, SContext* sc, MtlBaseLib* mtls)
{
	...
	if (ostate.obj->SuperClassID() == LIGHT_CLASS_ID) {
		...
		if (light->GetUseLight()) {
			...
			LightInfo *l = new LightInfo(node, mtls);
			sc->lightTab.Append(1, &l);
		}
	}
}
